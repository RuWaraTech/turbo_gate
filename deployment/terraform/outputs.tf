...
output "application_endpoints" {
  value = {
    ...
    backend_health = var.enable_load_balancer ? "http://${hcloud_server.manager.ipv4_address}:8080/waf-health" : "N/A"
    ssl_endpoint = var.enable_load_balancer ? "Load Balancer" : "Backend NGINX"
  }
...
output "deployment_summary" {
  value = {
    ...
    waf_enabled        = var.waf_enabled ? "Yes (backend HTTP)" : "No"
...
output "security_status" {
  value = {
    ...
    waf_protection    = var.waf_enabled ? "OWASP ModSecurity Active (backend)" : "Disabled"
